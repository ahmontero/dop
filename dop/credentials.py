...
class Credentials:
	...
	def load(self):
		with open(self.filename, 'r') as f:
			...
			for i in range(0, 3):
				password = getpass.getpass('Password to restore Digital Ocean credentials: ')
				try:
		...
		if self.client_id is None or self.api_key is None:
			raise RuntimeError('Failed to decrypt Digital Ocean credentials: bad password')
	...
	def save(self):
		with open(self.filename, 'w') as f:
			print("You're going to store Digital Ocean credentials in file: ", self.filename)
